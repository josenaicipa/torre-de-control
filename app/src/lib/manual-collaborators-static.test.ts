...
describe("manual collaborator labels", () => {
  ...
  it("shows Area Comercial por colaborador entries in Detalle Diario and Torre for June onward without rewriting closed May", () => {
    ...
    expect(torreBlock).toContain('const monthlyCommercialSalesDay=(date,closerRow)=>areaComercialHasPriority(date)?commercialValueByDay(date,"ventasHT"):(commercialValueByDay(date,"ventasHT")||((closerRow&&closerRow.q_ventas_ht)||0));');
    ...
    expect(detalleBlock).toContain('const cv=(d,closerField,entryField)=>areaComercialHasPriority(d.d)?d.sdCommercial(entryField):d.closer[closerField]||d.sdCommercial(entryField)||0;');
  ...
  it("sums Admin and closer Area Comercial entries from 2026-06-01 onward", () => {
    expect(html).toContain('const AREA_COMERCIAL_PRIORITY_START="2026-06-01";');
    expect(html).toContain('const areaComercialHasPriority=date=>date>=AREA_COMERCIAL_PRIORITY_START;');
    expect(html).toContain('const COMMERCIAL_REPORTING_MEMBER_IDS=new Set(COMMERCIAL_COLLABORATORS.map(c=>c.id));');
    expect(html).toContain('const commercialPrimaryEntry=entries=>(entries||[]).find(e=>e&&isAdminMember(e.member))||null;');
    expect(html).toContain('const commercialDayHasPrimary=(date,entries)=>false;');
    expect(html).toContain('const commercialDayValue=(date,entries,field)=>sumF(entries,field);');
    ...
    expect(html).toContain('const cv=(d,closerField,entryField)=>areaComercialHasPriority(d.d)?d.sdCommercial(entryField):d.closer[closerField]||d.sdCommercial(entryField)||0;');
    expect(html).toContain('const valorHTDay=d=>areaComercialHasPriority(d.d)?d.sdCommercial("valorVentaHT"):d.closer.valor_venta_ht||d.sdCommercial("valorVentaHT")||0;');
    expect(html).not.toContain('const commercialDayValue=(date,entries,field)=>commercialDayHasPrimary(date,entries)?nv(commercialPrimaryEntry(entries)[field]):sumF(entries,field);');
  ...
  it("sums commercial money metrics day-by-day in Detalle Diario and Torre CEO instead of choosing one monthly source", () => {
    ...
    expect(html).toContain('valorHT:commercialValorHTByDay[date]||0,');
    expect(html).toContain('cashHT:commercialCashByDay[date]||0,');
    expect(html).toContain('recurring:commercialRecurringByDay[date]||0,');
    expect(html).toContain('lowTicket:commercialLowTicketByDay[date]||0,');
    expect(html).toContain('reservas:commercialReservasByDay[date]||0,');
    ...
    expect(html).toContain('const dispReservas=monthlyCommercialMoneyDates.reduce((s,date)=>s+monthlyCommercialMoneyDay(date,allCloser[date]).reservas,0)||(cashApi?cashApi.reservas:0);');
    expect(html).toContain('const valorHTDay=d=>areaComercialHasPriority(d.d)?d.sdCommercial("valorVentaHT"):d.closer.valor_venta_ht||d.sdCommercial("valorVentaHT")||0;');
    expect(html).toContain('const cashHTDay=d=>areaComercialHasPriority(d.d)?(d.sdCommercial("cashCollected")||d.sdCommercial("upfrontCash")):d.closer.ventas_cash||d.sdCommercial("cashCollected")||d.sdCommercial("upfrontCash")||d.ledger.highTicket||0;');
    expect(html).toContain('const recurringDay=d=>areaComercialHasPriority(d.d)?d.sdCommercial("recurringCash"):d.closer.recurring_cash||d.sdCommercial("recurringCash")||0;');
    expect(html).toContain('const cashLowTicketDay=d=>areaComercialHasPriority(d.d)?d.sdCommercial("valorVentaLT"):d.closer.valor_venta_lt||d.sdCommercial("valorVentaLT")||d.ledger.lowTicket||0;');
    expect(html).toContain('const cashReservasDay=d=>areaComercialHasPriority(d.d)?d.sdCommercial("cashReservas"):d.closer.cash_reservas||d.sdCommercial("cashReservas")||d.ledger.reservas||0;');
    expect(html).toContain('{l:"$ Venta HT",fn:d=>valorHTDay(d)||null,fmt:"$"}');
